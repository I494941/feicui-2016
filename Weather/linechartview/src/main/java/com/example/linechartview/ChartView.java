    ...
    protected void onDraw(Canvas canvas) {
        ...
        for (int i = 0; i * XScale < XLength; i++) {
            ...
            try {
                ...
                //数据值
                if (i > 0 && YCoord(String.valueOf(Data[i - 1])) != -999 && YCoord(String.valueOf(Data[i])) != -999)  //保证有效数据
                    canvas.drawLine(XPoint + (i - 1) * XScale, YCoord(String.valueOf(Data[i - 1])), XPoint + i * XScale, YCoord(String.valueOf(Data[i])), paint);
                canvas.drawCircle(XPoint + i * XScale, YCoord(String.valueOf(Data[i])), 2, paint);
            } catch (Exception e) {
            }
        }
