...
//
    ...
    private void drawXY(Canvas canvas) {
        ...
        * */
        ...
        mPath.moveTo(viewSize * (3 / 32f), viewSize * (5 / 32f));
        mPath.lineTo(viewSize * (2 / 16f), viewSize * (2 / 16f));
        mPath.lineTo(viewSize * (5 / 32f), viewSize * (5 / 32f));
        ...
        mPath.moveTo(viewSize * (29 / 32f), viewSize * (15 / 32f));
        mPath.lineTo(viewSize * (15 / 16f), viewSize * (8 / 16f));
        mPath.lineTo(viewSize * (29 / 32f), viewSize * (17 / 32f));
    ...
    private void drawLines(Canvas canvas) {
        ...
        * */
        ...
        for (int i = 0; i < count; i++) {
            ...
            if (minY > pointFs.get(i).y) {
                minY = pointFs.get(i).y;
            }
        }
        Log.i("Text", "maxY:--" + maxY);
        Log.i("Text", "minY:--" + minY);
        // 计算纵轴最近的能被count整除的值
        int remainderY = ((int) (maxY - minY)) % divisor;
        Log.i("Text", "remainderY:--" + remainderY);

        if (remainderY == 0 && (maxY - minY) == 0) {
            maxY = 0;
        } else {
            // maxY = divisor - remainderY + ((int) (maxY - minY));
            maxY = maxY + (3 - maxY % 3);
            Log.d("maxY", "drawLines: " + maxY);
            if (minY >= 0) {
                minY = minY - minY % 3;
            } else {
                minY = minY + (-3 - minY % 3);
...
//        Log.i("Text","rulerX:--"+rulerX);
        ...
        for (int i = 0; i < count; i++) {
            rulerY[i] = maxY / divisor * i * 2 + minY;
            Log.d("rulerY[i]", "drawLines: " + rulerY[i]);
        }
        ...
        for (float y = buttom - spaceY; y > top; y -= spaceY) {
            for (float x = left; x < right; x += spaceX) {
                 ...
                 */
                if (x == left) {
                    canvas.drawText((int) (rulerY[num_y + 1]) + "", left - (left / 2), y + 10, mTextPaint);
    ...
    private void drawXYelement(Canvas canvas) {
        ...
        * */
        ...
        canvas.drawText("温度℃", PathY_X, PathY_Y, mTextPaint);
        ...
        * */
        ...
        canvas.drawText("日期", PathX_X, PathX_Y, mTextPaint);
        // 释放画布
        canvas.restore();
    }
    ...
    private void drawbitmaps(Canvas canvas) {
        ...
        * */
        ...
        Bitmap mBitmap = Bitmap.createBitmap((int) (right - left - spaceX), (int) (buttom - top - spaceY), Bitmap.Config.ARGB_8888);
    ...
    private void drawText(Canvas canvas) {
         ...
         */
            for (int i = 0; i < pointFs.size(); i++) {
                ...
                // 计算y坐标
                float y = mCanvas.getHeight() / maxY * (pointFs.get(i).y - minY) / 2;
